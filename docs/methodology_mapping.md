...
| Gaussian depth score (μ≈1250, σ≈350) | `scoring.score_depth_topo` |
| Gross thickness cutoff (≤100 m) and 0–10 ramp | `config.THICKNESS_*`; `scoring.score_thickness` |
| Insolubles scoring (~26% favorable, ~70% poor) | `config.INSOLUBLES_*`; `scoring.score_insolubles_percent` |
| Distance-to-fault cutoff (≤200 m) | `config.FAULT_DIST_*`; `scoring.score_fault_distance` |
...
| IDW interpolation of well-scale criteria | `grids.idw_interpolation`; `pipeline._interpolate_well_parameters` |
...
| Paper concept | Code location |
|---|---|
| Depth window 500–2000 m | `config.MIN_DEPTH`, `config.MAX_DEPTH`; `constraints.filter_depth_range` |
...
| Halite-to-halite boundary condition | `optimizer.get_halite_layers`, `optimizer.find_optimal_subinterval` |
...
| Insoluble exclusion (cavern-zone mean ≤35%) | `config.MAX_INSOLUBLES_PERCENT`; `optimizer.calculate_insolubles_at_position` |
| Sump correction (bulking factor 1.3) | `config.SWELLING_FACTOR`; `geometry.calculate_sump_volume` |
| Per-well Pareto frontier (net volume ↑, insolubles ↓) | `optimizer.generate_pareto_solutions`, `optimizer.calculate_pareto_frontier` |
...
| Scenarios HD=0.75 vs HD=2.00 from reference well Pareto analysis | `config.SCENARIOS` |
